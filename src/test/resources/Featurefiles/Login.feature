Feature: To validate the functionality of Metallica application
  ...
  @login 
	...
	@shippingpage
	Scenario: To validate all the fields in the Shipping Page
		When User navigate to shipping page
		And  User validate the mandatory fields
		And  User validate the shipping address fields
		And  User select address from saved address dropdown
		And  User click the Use this address checkbox
		And  User click the Is this a Gift checkbox
		Then User select the Shipping Method
		And  User check shipping method label for Pre Order product
		And  User click the Continue to billing button
		
	@billingpage
	Scenario: To validate all the fields in the Billing Page
		When User navigate to billing page
		And  User click review order button without filling the mandatory fields 
		And  User click the Paypal button
		And  User click the Edit button in SubTotal and Shipping address field
		And  User select the non taxable address from saved address dropdown
		And  User enter card details
		And  User validate the saved this card, add a gift and shipping address checkbox
